...
class Seq2seq(Model):
    ...
    def forward(self, inputs, seq_length=20, dec_len=None, enc_len=None, start_token=None, return_state=False, top_n=None):
        ...
        if (self.is_train):
            ...
            for i in range(self.n_layer):
                enc_output, state[i] = self.enc_layers[i](enc_output, sequence_length=enc_len, return_state=True)

            decoding = inputs[1]
            dec_output = self.embedding_layer(decoding)

            for i in range(self.n_layer):
                dec_output, state[i] = self.dec_layers[i](dec_output, sequence_length=dec_len, initial_state=state[i], return_state=True)
