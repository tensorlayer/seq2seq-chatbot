...
class Seq2seq_(Model):
    '''
    This version combines the last hidden state of the encoder, and the previous hidden state under the
    sigmoid function as the hidden state for next hidden state at decoder
    '''
    def __init__(
            self,
            batch_size,
        ...
        self.layer_for_hidden_state = tl.layers.Dense(n_units=cell_dec.units, in_channels=cell_dec.units)
    ...
    def forward(self, inputs, seq_length=30, start_token=None, return_state=False):
        ...
        if (self.is_train):
            ...
            total_steps = decoding.get_shape().as_list()[1]
            after_embedding_decoding = self.embedding_layer(decoding)
            ...
            for i in range(total_steps):
                cell_output, state = self.decoding_layer.cell.call(after_embedding_decoding[:, i, :], state, training=True)
                state = self.layer_for_hidden_state(state[0])
                state = tf.add(state, enc_state)
                dec_output.append(cell_output)

            # dec_rnn_output, state = self.decoding_layer(after_embedding_decoding, initial_state=enc_state, return_state=True)
